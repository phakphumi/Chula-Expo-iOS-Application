...
class FavViewController: UIViewController, UITableViewDelegate, UITableViewDataSource, NSFetchedResultsControllerDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if indexPath.section == 0 && indexPath.row == 0 {
            cell = tableView.dequeueReusableCell(withIdentifier: "HeaderMe", for: indexPath)
            if let headerCell = cell as? HeaderMeTableViewCell{
                headerCell.title1 = "UPCOMING EVENT ?"
                headerCell.title2 = "Events ที่กำลังจะเกิดขึ้น"
            ...
            cell = tableView.dequeueReusableCell(withIdentifier: "HeaderMe", for: indexPath)
            if let headerCell = cell as? HeaderMeTableViewCell{
                headerCell.title1 = "PREVIOUS EVENT"
                headerCell.title2 = "Events ที่เริ่มไปแล้ว"
                headerCell.iconImage = "heartIcon"
            }
            cell.selectionStyle = .none
        }
        else {
            cell = tableView.dequeueReusableCell(withIdentifier: "MeFeed", for: indexPath)
            if let fetchData = fetchedResultsController?.object(at: IndexPath(row: indexPath.row-1, section: 0)) as? ActivityData{
                ...
                if let eventFeedCell = cell as? FeedMeTableViewCell{
                    print("feedCell name == \(name)")
